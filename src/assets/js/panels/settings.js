...
class Settings {
    ...
    let ramRestanteSpan = document.getElementById("ram-restante");

    minSpan.setAttribute("value", `${ram.ramMin} Go`);
    maxSpan.setAttribute("value", `${ram.ramMax} Go`);

    // Affichage initial de la RAM restante
    let ramRestante = Math.max(0, totalMem - parseFloat(ram.ramMax));
    ramRestanteSpan.textContent = `${ramRestante.toFixed(1)} Go`;
    ...
    slider.on("change", async (min, max) => {
        ...
        // Met à jour la RAM restante dynamiquement
        let ramRestante = Math.max(0, totalMem - max);
        ramRestanteSpan.textContent = `${ramRestante.toFixed(1)} Go`;
